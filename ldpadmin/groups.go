...
func updateGroups(id string, json map[string]interface{}, tx *sql.Tx,
	opts *UpdateOptions) error {
	if json != nil {
		groupName := json["group"].(string)
		description := json["desc"].(string)
		_, err := exec(tx, opts, sqlUpdateGroups, id, groupName,
			description)
		return err
	} else {
		_, err := exec(tx, opts, sqlUpdateGroupsEmpty, id)
		return err
	}
}
